...
class Controller:
    ...
    def make(cls, environment, stages=None, **controller_kwargs):
        ...
        _controllers = {
            ...
            "cascaded_foc_rotor_flux_observer": [
                InductionMotorCascadedFieldOrientedController
            ],
        }
        ...
        if stages is not None:
            controller_type, stages = cls.find_controller_type(
                environment, stages, **controller_kwargs
            )
            assert (
                controller_type in _controllers.keys()
            ), f"Controller {controller_type} unknown"
            stages = cls.automated_gain(
                environment, stages, controller_type, _controllers, **controller_kwargs
            )
            controller = _controllers[controller_type][0](
                environment, stages, _controllers, **controller_kwargs
            )

        else:
            controller_type, stages = cls.automated_controller_design(
                environment, **controller_kwargs
            )
            stages = cls.automated_gain(
                environment, stages, controller_type, _controllers, **controller_kwargs
            )
    ...
    def reference_states(environment, **controller_kwargs):
        ...
        if isinstance(environment.reference_generator, MultipleReferenceGenerator):
            ...
            ref_states.append(
                environment.reference_generator._sub_generators[0]._reference_state
            )
        else:
    ...
    def find_controller_type(environment, stages, **controller_kwargs):
        ...
        if isinstance(environment.physical_system.unwrapped, DcMotorSystem):
            ...
            environment.physical_system.unwrapped, SquirrelCageInductionMotorSystem
        ):
            if len(stages) == 2:
                if len(stages[1]) == 1 and "i_sq" in controller_kwargs["ref_states"]:
            ...
            environment.physical_system.unwrapped, DoublyFedInductionMotorSystem
        ):
            if len(stages) == 2:
                if len(stages[1]) == 1 and "i_sq" in controller_kwargs["ref_states"]:
                    controller_type = "foc_rotor_flux_observer"
    ...
    def automated_controller_design(environment, **controller_kwargs):
        ...
        stages = []
        if isinstance(
            environment.physical_system.unwrapped, DcMotorSystem
        ):  # Checking type of motor
            if "omega" in ref_states or "torque" in ref_states:  # Checking control task
                ...
                for i in range(len(stages), 2):
                    if i == 0:
                        ...
                        )  # Adding PI-Controller for overlaid stages
            ...
            if "i_sq" in ref_states or "torque" in ref_states:
                controller_type = (
                    "foc_rotor_flux_observer"
                    if "i_sq" in ref_states
                    else "cascaded_foc_rotor_flux_observer"
                )
                if action_space_type is Discrete:
                    stages = [
                        [{"controller_type": "on_off"}],
    ...
    def automated_gain(
        ...
        if "automated_gain" not in controller_kwargs.keys() or automated_gain:
            ...
            if type(environment) in cont_extex_envs:
                ...
                p_gain = (
                    mp["l_e"]
                    / (environment.physical_system.tau * a)
                    / u_e_lim
                    * i_e_lim
                )
            ...
            if _controllers[controller_type][0] == ContinuousActionController:
                ...
                for i in range(len(stages)):
                    if type(stages_a[i]) is list:
                        if (
                            _controllers[stages_a[i][0]["controller_type"]][1]
                            == ContinuousController
                            ...
                            if i == 0:
                                ...
                                i_gain = p_gain / (
                                    environment.physical_system.tau * a**2
                                )

                                if (
                                    _controllers[stages_a[i][0]["controller_type"]][2]
                                    == PIDController
                                ):
                                    d_gain = p_gain * environment.physical_system.tau
                                    stages_a[i][0]["d_gain"] = stages_a[i][0].get(
                                        "d_gain", d_gain
                                    )
                            ...
                            stages_a[i][0]["p_gain"] = stages_a[i][0].get(
                                "p_gain", p_gain
                            )  # ?
                            stages_a[i][0]["i_gain"] = stages_a[i][0].get(
                                "i_gain", i_gain
                            )  # ?

                    elif type(stages_a[i]) is dict:
                        if (
                            _controllers[stages_a[i]["controller_type"]][1]
                            == ContinuousController
                        ):  # had to add [0] to make dict in list acessable
                            if i == 0:
                                p_gain = (
                                ...
                                if (
                                    _controllers[stages_a[i]["controller_type"]][2]
                                    == PIDController
                                ):
                                    d_gain = p_gain * environment.physical_system.tau
                                    stages_a[i]["d_gain"] = stages_a[i].get(
                                        "d_gain", d_gain
                                    )
                            ...
                            stages_a[i]["p_gain"] = stages_a[i].get(
                                "p_gain", p_gain
                            )  # ?
                            stages_a[i]["i_gain"] = stages_a[i].get(
                                "i_gain", i_gain
                            )  # ?
                ...
                if type(environment.action_space) == Box:
                    ...
                    if (
                        "i_sq" in ref_states
                        and _controllers[stage_q["controller_type"]][1]
                        == ContinuousController
                    ):
                        p_gain_d = (
                            mp["l_d"]
                            / (1.5 * environment.physical_system.tau * a)
                            / u_sd_lim
                            * i_sd_lim
                        )
                        i_gain_d = p_gain_d / (
                            1.5 * environment.physical_system.tau * a**2
                        )
                ...
                if type(environment.action_space) is Box:
                    ...
                    if (
                        ...
                        and _controllers[overlaid[0]["controller_type"]][1]
                        == ContinuousController
                    ):
                        ...
                        if (
                            _controllers[overlaid[0]["controller_type"]][2]
                            == PIDController
                        ):
                            d_gain = p_gain * environment.physical_system.tau
                    ...
                    if (
                        "omega" in ref_states
                        and _controllers[stages[3][0]["controller_type"]][1]
                        == ContinuousController
                    ):
                        p_gain = (
                            environment.physical_system.mechanical_load.j_total
                        ...
                        if (
                            _controllers[stages[3][0]["controller_type"]][2]
                            == PIDController
                        ):
                            d_gain = p_gain * environment.physical_system.tau
                ...
                _controllers[controller_type][0]
                == InductionMotorFieldOrientedController
            ):
                mp["l_s"] = mp["l_m"] + mp["l_sigs"]
                mp["l_r"] = mp["l_m"] + mp["l_sigr"]
                sigma = (mp["l_s"] * mp["l_r"] - mp["l_m"] ** 2) / (
                    mp["l_s"] * mp["l_r"]
                )
                tau_sigma = (sigma * mp["l_s"]) / (
                    mp["r_s"] + mp["r_r"] * mp["l_m"] ** 2 / mp["l_r"] ** 2
                )
                tau_r = mp["l_r"] / mp["r_r"]
