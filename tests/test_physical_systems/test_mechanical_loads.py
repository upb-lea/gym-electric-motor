...
def speed_profile(t, amp, freq, bias):
...
class TestExtSpeedLoad(TestMechanicalLoad):
    ...
    kwargs = dict(speed_profile=speed_profile,
                  amp=test_amp,
                  bias=test_bias,
                  freq=test_freq)
    ...
    def ext_speed_load(self):
        return ExternalSpeedLoad(speed_profile=speed_profile,
                                 amp=test_amp, bias=test_bias, freq=test_freq)

    def test_initialization(self):
        load = ExternalSpeedLoad(speed_profile=speed_profile,
                                 amp=test_amp, bias=test_bias, freq=test_freq)
        assert load._speed_profile == speed_profile
        assert load.omega == speed_profile(t=0,
                                                amp=test_amp,
                                                bias=test_bias,
                                                freq=test_freq)
    ...
    def test_jacobian(self, omega, omega_initial, expected):
        test_object = self.class_to_test(speed_profile, amp=test_amp,
