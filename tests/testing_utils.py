...
def setup_physical_system(
    motor_type, converter_type, subconverters=None, three_phase=False
):
    """
    ...
    if three_phase:
        if motor_type == "SCIM":
            ...
            physical_system = DoublyFedInductionMotor(
                converter=converter,
                motor=motor,
                ode_solver=solver,
                supply=voltage_supply,
...
def setup_reference_generator(reference_type, physical_system, reference_state="omega"):
    ...
    """
    reference_generator = make_module(
        ReferenceGenerator, reference_type, reference_state=reference_state
    )
