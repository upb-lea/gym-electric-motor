...
def test_registry(monkeypatch):
    ...
    utils.register_class(
        dummies.DummyPhysicalSystem, core.PhysicalSystem, "DummySystem"
    )
...
def test_instantiate(monkeypatch):
    ...
    instance = utils.instantiate(
        core.PhysicalSystem, dummies.DummyPhysicalSystem, **kwargs
    )
    ...
    key = "DummyKey"
    assert utils.instantiate(core.PhysicalSystem, key, **kwargs) == (
        core.PhysicalSystem,
        key,
        kwargs,
    )
    ...
    with pytest.raises(Exception):
        utils.instantiate(
            core.ReferenceGenerator, dummies.DummyPhysicalSystem, **kwargs
        )
    with pytest.raises(Exception):
        utils.instantiate(
            dummies.DummyPhysicalSystem, core.ReferenceGenerator, **kwargs
        )
    with pytest.raises(Exception):
        utils.instantiate(
            dummies.DummyPhysicalSystem(), core.ReferenceGenerator, **kwargs
