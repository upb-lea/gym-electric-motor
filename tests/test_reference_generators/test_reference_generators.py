...
class TestSwitchedReferenceGenerator:
    ...
    def monkey_instantiate(self, superclass, instance, **kwargs):
        ...
        """
        assert (
            superclass == ReferenceGenerator
        ), "superclass is not ReferenceGenerator as expected"
        assert (
    ...
    def monkey_super_set_modules(self, physical_system):
        ...
        ), "physical system is not the expected instance"
    ...
    def monkey_dummy_reset(self, initial_state, initial_reference):
        ...
        if type(initial_state == self._initial_state) is bool:
            ...
            ), "passed initial state is not the expected one"
        else:
            assert all(
                initial_state == self._initial_state
            ), "passed initial state is not the expected one"
        ...
        ), "passed initial reference is not the expected one"
    ...
    def monkey_dummy_get_reference(self, state, **kwargs):
        ...
        assert (
            self._kwargs == kwargs
        ), "Different additional arguments. Keep in mind None and {}."
    ...
    def test_init(
        self, monkeypatch, setup, sub_generator, p, super_episode_length, expected_sel
    ):
        """
        ...
        assert len(test_object._sub_generators) == len(
            sub_generator
        ), "unexpected number of sub generators"
        assert (
            test_object._current_episode_length == 0
        ), "The current episode length is not 0."
        assert (
            test_object._super_episode_length == expected_sel
        ), "super episode length is not as expected"
        assert test_object._current_ref_generator in sub_generator
        assert test_object._sub_generators == list(
            sub_generator
        ), "Other sub generators than expected"
    ...
    def test_set_modules(self, monkeypatch, setup):
        ...
        self._physical_system._state_space = gymnasium.spaces.Box(
            -1, 1, shape=self._physical_system.state_space.shape
        )
        # call function to test
        test_object.set_modules(self._physical_system)
        # verify the expected results
        assert all(
            test_object.reference_space.low == expected_space.low
        ), "Lower limit of the reference space is not 0"
        assert (
            test_object.reference_space.high == expected_space.high
        ), "Upper limit of the reference space is not 1"
        assert np.all(
            test_object._referenced_states == reference_states
        ), "referenced states are not the expected ones"
    ...
    def test_reset(self, monkeypatch, setup, initial_state, initial_reference):
        ...
        # setup test scenario
        monkeypatch.setattr(
            SwitchedReferenceGenerator, "_reset_reference", self.monkey_reset_reference
        )
        sub_generator = [
            SinusoidalReferenceGenerator(),
            WienerProcessReferenceGenerator(),
        ...
        test_object = SwitchedReferenceGenerator(sub_generator)
        monkeypatch.setattr(
            test_object._sub_generators[0], "reset", self.monkey_dummy_reset
        )
        ...
        assert (
            self._monkey_dummy_reset_counter == 1
        ), "reset of sub generators is not called once"
        assert res_0 == self._reference, "reference is not the expected one"
        assert all(
            res_1 == self._reference_observation
        ), "observation is not the expected one "
        assert (
    ...
    def test_get_reference(self, monkeypatch):
        ...
        test_object = SwitchedReferenceGenerator(sub_generator)
        monkeypatch.setattr(
            DummyReferenceGenerator, "get_reference", self.monkey_dummy_get_reference
        )
        # call function to test
    ...
    def test_get_reference_observation(
        self, monkeypatch, k, current_episode_length, k_new, reset_counter
    ):
        """
        ...
        monkeypatch.setattr(
            test_object, "_current_episode_length", current_episode_length
        )
        monkeypatch.setattr(
            test_object, "_reset_reference", self.monkey_reset_reference
        )
        monkeypatch.setattr(test_object, "_reference", self._initial_reference)
        monkeypatch.setattr(
            test_object._current_ref_generator, "reset", self.monkey_dummy_reset
        )
        monkeypatch.setattr(
        ...
        # call function to test
        observation = test_object.get_reference_observation(
            self._initial_state, **self._kwargs
        )
        # verify the expected results
        assert all(
            observation == self._reference_observation
        ), "observation is not the expected one"
        assert test_object._k == k_new, "unexpected new step in the reference"
        assert (
            self._monkey_reset_reference_counter == reset_counter
        ), "reset_reference called unexpected often"
...
class TestWienerProcessReferenceGenerator:
    ...
    def test_init(self, monkeypatch, sigma_range):
        ...
        assert (
            test_object._sigma_range == sigma_range
        ), "sigma range is not passed correctly"
    ...
    def test_reset_reference(self, monkeypatch):
        ...
        test_object._reset_reference()
        assert (
            sum(abs(test_object._reference - expected_reference)) < 1e-6
        ), "unexpected reference array"
        assert (
            self._monkey_get_current_value_counter == 1
        ), "get_current_value() not called once"
...
class TestFurtherReferenceGenerator:
    ...
    def test_init(
        ...
        # verify the expected results
        assert (
            test_object._amplitude_range == amplitude_range
        ), "amplitude range is not passed correctly"
        assert (
            test_object._frequency_range == frequency_range
        ), "frequency range is not passed correctly"
        assert (
            test_object._offset_range == offset_range
        ), "offset range is not passed correctly"
    ...
    def test_set_modules(
        ...
        monkeypatch.setattr(
            SubepisodedReferenceGenerator, "set_modules", self.monkey_super_set_modules
        )
        test_object = reference_class(
            amplitude_range=amplitude_range, offset_range=offset_range
        )
        ...
        # verify the expected results
        assert all(
            test_object._amplitude_range == expected_amplitude
        ), "amplitude range is not as expected"
        assert all(
            test_object._offset_range == expected_offset
        ), "offset range is not as expected"
        assert (
            self._monkey_super_set_modules_counter == 1
        ), "super().set_modules() is not called once"
    ...
    def test_reset_reference(
        self, monkeypatch, reference_class, expected_reference, frequency_range
    ):
        # setup test scenario
        amplitude_range = 0.8
        offset_range = 0.5
        ...
        assert (
            sum(abs(expected_reference - test_object._reference)) < 1e-6
...
class TestSubepisodedReferenceGenerator:
    ...
    def test_init(self, monkeypatch, limit_margin):
        ...
        # verify the expected results
        assert (
            test_object._limit_margin == limit_margin
        ), "limit margin is not passed correctly"
        assert test_object._reference_value == 0.0, "the reference value is not 0"
        assert (
            test_object._reference_state == self._reference_state
        ), "reference state is not passed correctly"
        assert (
            test_object._episode_len_range == self._episode_length
        ), "episode length is not passed correctly"
        assert (
            test_object._current_episode_length == self._current_value
        ), "current episode length is not as expected"
    ...
    def test_set_modules(self, monkeypatch, limit_margin, expected_low, expected_high):
        ...
        # setup test scenario
        monkeypatch.setattr(
            ReferenceGenerator, "set_modules", self.monkey_super_set_modules
        )
        monkeypatch.setattr(srg, "set_state_array", self.monkey_state_array)
        test_object = SubepisodedReferenceGenerator(
            reference_state=self._reference_state, limit_margin=limit_margin
        )
        # call function to test
        test_object.set_modules(self._physical_system)
        # verify the expected results
        ...
        assert all(
            test_object.reference_space.low == expected_low
        ), "lower reference space not as expected"
        assert all(
            test_object.reference_space.high == expected_high
        ), "upper reference space not as expected"
    ...
    def test_reset(
        self, monkeypatch, initial_reference, initial_state, expected_reference
    ):
        """
        test reset()
        :param monkeypatch:
        ...
        assert (
            test_object._current_episode_length == -1
        ), "current episode length is not -1"
        assert (
            test_object._reference_value == expected_reference
        ), "unexpected reference value"
    ...
    # setup test scenario
    def test_get_reference_observation(
        self, monkeypatch, k, expected_reference, expected_parameter
    ):
        """
        test get_reference_observation()
        :param monkeypatch:
        ...
        assert (
            test_object._current_episode_length == expected_parameter[0]
        ), "unexpected current episode length"
        assert (
    ...
    def test_get_current_value(self, monkeypatch, value_range, expected_value):
        ...
        # verify expected results
        assert (
            abs(val - expected_value) < 1e-6
        ), "unexpected value from get_current_value()."
...
class TestConstReferenceGenerator(TestReferenceGenerator):
    ...
    def reference_generator(self, physical_system):
        rg = ConstReferenceGenerator(
            reference_state=physical_system.state_names[1], reference_value=1.0
        )
    ...
    def test_initialization(self, reference_value, reference_state):
        rg = ConstReferenceGenerator(
            reference_value=reference_value, reference_state=reference_state
        )
        assert rg.reference_space == Box(
            np.array([reference_value]), np.array([reference_value]), dtype=np.float64
        )
...
class TestMultipleReferenceGenerator(TestReferenceGenerator):
    ...
    def test_reference_observation(self, reference_generator):
        assert all(
            reference_generator.get_reference_observation(0) == np.array([-1, 1])
        )

    def test_registered(self):
        if self.key != "":
            rg = gem.utils.instantiate(
                ReferenceGenerator, self.key, sub_generators=[DummyReferenceGenerator]
            )
            assert type(rg) == self.class_to_test
