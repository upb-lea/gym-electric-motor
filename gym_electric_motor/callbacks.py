...
class RampingLimitMargin(Callback):
    ...
    def __init__(
        ...
        assert (
            initial_limit_margin[0] >= -1
        ), "Lower limit margin has to be bigger than or equal to -1"
        assert (
            maximum_limit_margin[0] >= -1
        ), "Lower limit margin has to be bigger than or equal to -1"
        assert (
            initial_limit_margin[1] <= 1
        ), "Upper limit margin has to be smaller than or equal to 1"
        assert (
            maximum_limit_margin[1] <= 1
        ), "Upper limit margin has to be smaller than or equal to 1"
    ...
    def set_env(self, env):
        ...
        if isinstance(env.reference_generator, SwitchedReferenceGenerator):
            for sub_generator in env.reference_generator._sub_generators:
                assert issubclass(
                    type(sub_generator), SubepisodedReferenceGenerator
                ), self.__CLASS_ERROR__
        else:
            assert issubclass(
                type(env.reference_generator), SubepisodedReferenceGenerator
            ), self.__CLASS_ERROR__
        self._env = env
    ...
    def _update_limit_margin(self):
        ...
        if self._limit_margin != self._maximum_limit_margin:
            ...
            new_lower_limit = (
                new_lower_limit
                if new_lower_limit > self._maximum_limit_margin[0]
                else self._maximum_limit_margin[0]
            )
            new_upper_limit = self._limit_margin[1] + self._step_size
            new_upper_limit = (
                new_upper_limit
                if new_upper_limit < self._maximum_limit_margin[1]
                else self._maximum_limit_margin[1]
            )
