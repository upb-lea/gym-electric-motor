...
class SawtoothReferenceGenerator(SubepisodedReferenceGenerator):
    ...
    def __init__(
        self, amplitude_range=None, frequency_range=(1, 10), offset_range=None, **kwargs
    ):
        """
        Args:
    ...
    def set_modules(self, physical_system):
        ...
        self._offset_range = np.clip(
            self._offset_range, self._limit_margin[0], self._limit_margin[1]
        )
    ...
    def _reset_reference(self):
        ...
        self._reference = (
            self._amplitude * sg.sawtooth(2 * np.pi * self._frequency * t + phase)
            + self._offset
        )
        self._reference = np.clip(
            self._reference, self._limit_margin[0], self._limit_margin[1]
        )
