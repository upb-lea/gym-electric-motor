...
class MultipleReferenceGenerator(ReferenceGenerator, gem.RandomComponent):
    ...
    def set_modules(self, physical_system):
        ...
        assert all(
            sum(
                [
                    sub_generator.referenced_states.astype(int)
                    for sub_generator in self._sub_generators
                ]
            )
            < 2
        ...
        ref_space_low = np.concatenate(
            [
                sub_generator.reference_space.low
                for sub_generator in self._sub_generators
            ]
        )
        ref_space_high = np.concatenate(
            [
        ...
        self._referenced_states = np.sum(
            [sub_generator.referenced_states for sub_generator in self._sub_generators],
            dtype=bool,
            axis=0,
        )
    ...
    def reset(self, initial_state=None, initial_reference=None):
        ...
        ref_obs = np.array([])
        for sub_generator in self._sub_generators:
            ref, ref_observation, _ = sub_generator.reset(
    ...
    def get_reference(self, state, **kwargs):
        ...
        return sum(
            [
                sub_generator.get_reference(state, **kwargs)
                for sub_generator in self._sub_generators
            ]
        )
    ...
    def get_reference_observation(self, state, *_, **kwargs):
        ...
        return np.concatenate(
            [
                sub_generator.get_reference_observation(state, **kwargs)
                for sub_generator in self._sub_generators
            ]
        )
