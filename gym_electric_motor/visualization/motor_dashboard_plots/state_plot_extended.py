...
class StatePlotExtended(TimePlot):
    ...
    def initialize(self, axis, ref=dict(ref_state=[])):
        # Docstring of superclass
        ...
        if self._state_idx in ref['ref_state']:
            self._referenced = True
            color = 'orange' if 'color' not in ref else ref['color']

        if self._referenced:
            self._reference_line, = self._axis.plot(self._x_data, self._ref_data, color=color, **self._ref_line_config)
    ...
    def update_reference(self, ref=dict(ref_state=[], ref_value=[])):
        ref_state = ref['ref_state']
        ref_value = ref['ref_value']
        if self._state_idx in ref_state:
            self._ref_data[self.k-1] = ref_value[ref_state.index(self._state_idx)] * self._limits
