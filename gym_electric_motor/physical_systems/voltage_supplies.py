...
class MyRCVoltageSupply(VoltageSupply):
    ...
    def __init__(self, u_nominal=600.0, supply_parameter=None):
        ...
        assert (
            "R" in supply_parameter.keys()
        ), "Pass key 'R' for Resistance in your dict"
        assert (
            "C" in supply_parameter.keys()
        ), "Pass key 'C' for Capacitance in your dict"
...
class AC1PhaseSupply(VoltageSupply):
    ...
    def __init__(self, u_nominal=230, supply_parameter=None):
        ...
        if supply_parameter is not None:
            assert isinstance(
                supply_parameter, dict
            ), "supply_parameter should be a dict"
            assert (
                "frequency" in supply_parameter.keys()
            ), "Pass key 'frequency' for frequency f in Hz in your dict"
...
class AC3PhaseSupply(VoltageSupply):
    ...
    def get_voltage(self, t, *_, **__):
        ...
        self._u_sup = [
            self._max_amp
            * np.sin(2 * np.pi * self._f * t + self._phi + 2 / 3 * np.pi * i)
            for i in range(3)
        ]
        return self._u_sup
