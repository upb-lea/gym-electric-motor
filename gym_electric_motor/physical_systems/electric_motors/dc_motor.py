...
class DcMotor(ElectricMotor):
    ...
    _default_nominal_values = dict(
        omega=300, torque=16.0, i=97, i_a=97, i_e=97, u=60, u_a=60, u_e=60
    )
    _default_limits = dict(
        omega=400, torque=38.0, i=210, i_a=210, i_e=210, u=60, u_a=60, u_e=60
    )
    ...
    def _update_model(self):
        ...
        self._model_constants = np.array(
            [[-mp["r_a"], 0, -mp["l_e_prime"], 1, 0], [0, -mp["r_e"], 0, 0, 1]]
        )
        self._model_constants[self.I_A_IDX] = (
    ...
    def get_state_space(self, input_currents, input_voltages):
        ...
        low = {
            "omega": -1
            if input_voltages.low[a_converter] == -1
            or input_voltages.low[e_converter] == -1
            else 0,
            "torque": -1
            if input_currents.low[a_converter] == -1
            or input_currents.low[e_converter] == -1
            else 0,
    ...
    def _update_limits(self, limits_d=None, nominal_d=None):
        ...
        limits_d.update(
            dict(torque=self.torque([self._limits[state] for state in self.CURRENTS]))
        )
