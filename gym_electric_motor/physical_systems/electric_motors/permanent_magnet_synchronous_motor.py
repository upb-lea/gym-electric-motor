...
class PermanentMagnetSynchronousMotor(SynchronousMotor):
    ...
    _default_limits = dict(
        omega=4e3 * np.pi / 30, torque=0.0, i=400, epsilon=math.pi, u=300
    )
    _default_nominal_values = dict(
        omega=3e3 * np.pi / 30, torque=0.0, i=240, epsilon=math.pi, u=300
    )
        ...
        return (
            ...
            np.array(
                [  # dT/dx
                    ...
                    1.5
                    * mp["p"]
                    * (mp["psi_p"] + (mp["l_d"] - mp["l_q"]) * state[self.I_SD_IDX]),
                    0,
                ]
            ),
        )
