...
class DoublyFedInductionMotor(InductionMotor):
    ...
    _default_limits = dict(
        omega=1800 * np.pi / 30, torque=0.0, i=9, epsilon=math.pi, u=720
    )
    _default_nominal_values = dict(
        omega=1650 * np.pi / 30, torque=0.0, i=7.5, epsilon=math.pi, u=720
    )
    ...
    def _update_limits(self, limit_values={}, nominal_values={}):
        ...
        for u, i in zip(
            ...
            limits_agenda[i] = (
                ...
                or self._limits[u] / self._motor_parameter["r_r"]
            )
            nominal_agenda[i] = (
                self._nominal_values.get("i", None)
                or self._nominal_values[u] / self._motor_parameter["r_r"]
            )
    ...
    def _update_initial_limits(self, nominal_new={}, omega=None):
        ...
        flux_nominal_limits = {
            state: value for state, value in zip(self.FLUXES, flux_alphabeta_limits)
        }
