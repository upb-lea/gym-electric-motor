...
class SquirrelCageInductionMotor(InductionMotor):
    ...
    _default_limits = dict(
        omega=4e3 * np.pi / 30, torque=0.0, i=5.5, epsilon=math.pi, u=560
    )
    _default_nominal_values = dict(
        omega=3e3 * np.pi / 30, torque=0.0, i=3.9, epsilon=math.pi, u=560
    )
    ...
    def _update_initial_limits(self, nominal_new={}, omega=None):
        ...
        flux_alphabeta_limits = self._flux_limit(
            omega=omega, eps_mag=eps_mag, u_q_max=self._nominal_values["u_sq"]
        )
