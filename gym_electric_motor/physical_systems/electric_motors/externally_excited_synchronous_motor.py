...
class ExternallyExcitedSynchronousMotor(SynchronousMotor):
    ...
    _default_limits = dict(
        omega=12e3 * np.pi / 30, torque=0.0, i=150, i_e=150, epsilon=math.pi, u=320
    )
    _default_nominal_values = dict(
        omega=4.3e3 * np.pi / 30, torque=0.0, i=120, i_e=150, epsilon=math.pi, u=320
    )
    ...
    def _update_model(self):
        ...
        self._model_constants[self.I_SD_IDX] = (
            self._model_constants[self.I_SD_IDX] / mp["l_d"]
        )
        self._model_constants[self.I_SQ_IDX] = (
        ...
        return (
            np.array(
                [  # dx'/dx
                    [
                        -mp["r_s"] / mp["l_d"],
                        mp["l_q"] / (sigma * mp["l_d"]) * omega * mp["p"],
                        mp["l_m"] * mp["r_e"] / (sigma * mp["l_d"] * mp["l_e"]),
                        0,
                    ],
                    [
                        -mp["l_d"] / mp["l_q"] * omega * mp["p"],
                        -mp["r_s"] / mp["l_q"],
                        -omega * mp["p"] * mp["l_e"] / mp["l_q"],
                        0,
                    ],
                    [
                        mp["l_m"] * mp["r_s"] / (sigma * mp["l_d"] * mp["l_e"]),
                        -omega
                        * mp["p"]
                        * mp["l_m"]
                        * mp["l_q"]
            ...
            np.array(
                [  # dx'/dw
                    ...
                    - mp["p"] * mp["l_m"] / mp["l_q"] * state[self.I_E_IDX],
                    -mp["p"],
                    mp["p"],
                ]
            ),
