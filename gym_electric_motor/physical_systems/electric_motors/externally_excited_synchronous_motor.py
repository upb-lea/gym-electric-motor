...
class ExternallyExcitedSynchronousMotor(SynchronousMotor):
    ...
    def _update_model(self):
        ...
        self._model_constants = np.array([
            #                 omega,        i_d,        i_q,         i_f, u_d, u_q, u_f           omega * i_d,          omega * i_q, omega * i_f
            [                     0, -mp['r_s'],          0,           0,   1,   0,   0,                    0,  mp['l_q'] * mp['p'],                    0],
            [                     0,          0, -mp['r_s'],           0,   0,   1,   0, -mp['l_d'] * mp['p'],                    0, -mp['p'] * mp['l_f']],
            [                     0,          0,          0,  -mp['r_e'],   0,   0,   1,                    0,                    0,                    0],
    ...
    def torque(self, currents):
        # Docstring of superclass
        mp = self._motor_parameter
        return 1.5 * mp['p'] * (mp['l_e'] * currents[self.I_E_IDX] + (mp['l_d'] - mp['l_q']) * currents[self.I_SD_IDX]) * currents[self.I_SQ_IDX]

    def electrical_jacobian(self, state, u_in, omega, *args):
        mp = self._motor_parameter
        return (
            np.array([ # dx'/dx
                [                  -mp['r_s'] / mp['l_d'], mp['l_q'] / mp['l_d'] * omega * mp['p'],                                        0, 0],
