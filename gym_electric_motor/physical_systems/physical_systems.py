...
class SCMLSystem(PhysicalSystem, RandomComponent):
    ...
    def __init__(
        self, converter, motor, load, supply, ode_solver, tau=1e-4, calc_jacobian=None
    ):
        """
        Args:
        ...
        if calc_jacobian is None:
            calc_jacobian = (
                self._electrical_motor.HAS_JACOBIAN
                and self._mechanical_load.HAS_JACOBIAN
            )
        if (
            calc_jacobian
            and self._electrical_motor.HAS_JACOBIAN
            and self._mechanical_load.HAS_JACOBIAN
        ):
        ...
        if calc_jacobian and jac is None:
            warnings.warn(
                "Jacobian Matrix is not provided for either the motor or the load Model"
            )
    ...
    def _set_indices(self):
        ...
        self.U_SUP_IDX = list(
            range(voltages_upper, voltages_upper + self._supply.voltage_len)
        )
    ...
    def simulate(self, action, *_, **__):
        ...
        self.system_state[self.CURRENTS_IDX] = motor_state[
            self._electrical_motor.CURRENTS_IDX
        ]
    ...
    def _system_equation(self, t, state, u_in, **__):
        ...
        if self._system_eq_placeholder is None:
            ...
            self._system_eq_placeholder = np.concatenate(
                (load_derivative, motor_derivative)
            )
    ...
    def _system_jacobian(self, t, state, u_in, **__):
        ...
        ) = self._electrical_motor.electrical_jacobian(
            motor_state, u_in, state[self._omega_ode_idx]
        )
        torque = self._electrical_motor.torque(motor_state)
    ...
    def reset(self, *_):
        ...
        motor_state = self._electrical_motor.reset(
            state_space=self.state_space, state_positions=self.state_positions
        )
...
class ThreePhaseMotorSystem(SCMLSystem):
    ...
    def dq_to_abc_space(self, dq_quantities, epsilon_el, normed_epsilon=False):
        ...
        if normed_epsilon:
            epsilon_el *= np.pi
        return self._electrical_motor.t_32(
            self._electrical_motor.q(dq_quantities, epsilon_el)
        )

    def alphabeta_to_dq_space(
        self, alphabeta_quantities, epsilon_el, normed_epsilon=False
    ):
        """
...
class SynchronousMotorSystem(ThreePhaseMotorSystem):
    ...
    def simulate(self, action, *_, **__):
        ...
        i_in = self.dq_to_abc_space(
            self._electrical_motor.i_in(ode_state[self._ode_currents_idx]), eps
        )
        switching_times = self._converter.set_action(action, self._t)
        ...
        system_state = np.concatenate(
            (mechanical_state, [torque], i_abc, i_dq, u_in, u_dq, [eps], u_sup)
        )
        return system_state / self._limits
...
class ExternallyExcitedSynchronousMotorSystem(SynchronousMotorSystem):
    ...
    def simulate(self, action, *_, **__):
        ...
        i_in_abc_e = list(self.dq_to_abc_space(i_in_dq_e[:2], eps)) + list(
            i_in_dq_e[2:]
        )
        switching_times = self._converter.set_action(action, self._t)
        ...
        system_state = np.concatenate(
            (mechanical_state, [torque], i_abc, i_dq_e, u_in[:3], u_dq_e, [eps], u_sup)
        )
        return system_state / self._limits
...
class SquirrelCageInductionMotorSystem(ThreePhaseMotorSystem):
    ...
    def _set_indices(self):
        ...
        self._ode_flux_idx = self._motor_ode_idx[
            self._electrical_motor.PSI_RALPHA_IDX : self._electrical_motor.PSI_RBETA_IDX
            + 1
        ]
    ...
    def reset(self, *_):
        ...
        self._ode_solver.set_initial_value(ode_state, self._t)
        system_state = np.concatenate(
            [mechanical_state, [torque], i_abc, i_dq, u_abc, u_dq, [eps], u_sup]
        )
        return system_state / self._limits
...
class DoublyFedInductionMotorSystem(ThreePhaseMotorSystem):
    ...
    def __init__(self, ode_solver="scipy.ode", **kwargs):
        ...
        self.stator_voltage_high_idx = (
            ...
            + self._converter.subsignal_voltage_space_dims[
                self.stator_voltage_space_idx
            ]
        )
    ...
    def simulate(self, action, *_, **__):
        ...
        i_sabc = self.alphabeta_to_abc_space(
            self._electrical_motor.i_in(ode_state[self._ode_currents_idx])
        )
        ...
        for t in switching_times[:-1]:
            ...
            u_sup = self._supply.get_voltage(self._t, i_sup)
            u_in = self._converter.convert(
                np.concatenate([i_sabc, i_rdef]).tolist(), self._ode_solver.t
            )
            u_in = [u * u_s for u in u_in for u_s in u_sup]
            ...
            i_rdef = self.alphabeta_to_abc_space(
                self.calculate_rotor_current(ode_state)
            )
